...
[#expressions]
== Expressions
Module components typically use <<Expression.adoc#top,Expressions>> to determine the conditions under which
the effects of the component should apply. There are several kinds of expressions, including:

[#beanshell]
=== BeanShell Expressions
Modern module components usually use the <<Expression.adoc#beanshell,BeanShell Expression>> syntax to define a value calculated from an arbitrarily complex combination of <<#properties,Properties>> and other values defined within the module.

[#oldStyle]
=== Old-Style Expressions
Also known as <<Expression.adoc#simple,Simple Expressions>>, this is the expression syntax that was originally used in early versions of Vassal. While still supported, this syntax is now consdered obsolete so it is recommended that BeanShell expressions be used when creating or modifying module components.

...
+
*_Markers_* and *_Dynamic Properties_* are handled slightly differently.
+
A _Marker_ defined in the replacement piece will always get the value defined in the replacement piece, even if the same _Marker__ is defined in the source piece.
+
_Dynamic Properties_ are just compared on the property name, not the full range of options (e.g. minimum value).  This makes it easy to copy the value of a _Dynamic Property_ into the replacement piece. If you want to use 'Match State', but do not want to copy the value of a _Dynamic Property_, then move the _Dynamic Property_ trait in the source piece below the _Replace With Other_ trait and select the 'Only match states above this trait' option.

...
As of VASSAL 3.7, the property _MostRecentMatches_ can be checked following a Global Key Command to find out how many pieces the command was sent to. This property is only valid during an execution sequence within  a piece (i.e. the piece receives a key command, sends a GKC, and then is still further processing part of the original command). It does not remain valid e.g. in between user commands, nor is it saved when saving the game, etc. Think of it as a transient property to be checked only immediately downstream of sending a Global Key Command.
...
*Among pieces found, limit to:*::  Allows the Global Key Command to be restricted to affecting a pre-specified maximum number of pieces. The default, _All Matching Pieces_, does not limit the total number. If _Specified Number of Matching Pieces_ is selected, then an expression can be provided to set the maximum number of targets, and once that number of targets has been reached the Global Key Command will stop executing. Note that any piece which matches the fast match parameters plus additional matching expression (and e.g. range, if applicable) will be counted toward the total, _regardless of whether or not the piece has a trait that "does anything" based on the key command sent to it._

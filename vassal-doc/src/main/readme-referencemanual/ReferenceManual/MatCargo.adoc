...
*Key Command to force attach to any mat found at current location:*:: If the piece receives this key command, it will immediately search for (and attempt to attach itself to) any available mat at its current location. Note that attachment is normally handled automatically whenever a cargo piece is moved, but this allows a check to be forced explicitly in special situations.

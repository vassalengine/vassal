...
(6) Attachments are also allowed to be made from a piece to itself (`Allow attachments to self'). This allows you to create a group of pieces that are all equivalently attached to all other piecs in the group and can all issue the same Attachment related commands. By default, this option is disabled, which creates an assymetric Attachment group, where one piece is the 'driver' that sends commands to an attached set of pieces, but they cannot necessarily send commands back.

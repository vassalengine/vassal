...
NOTE: A checkpoint hotkey, if set, allows the module designer to prevent undo following a key action, such as after a card draw by issuing the specified hotkey at the _end_ of the action sequence. This feature is not intended for direct player use and the Keystroke Command option is provided for testing only.

...
As of VASSAL 3.7, a startup global _hotkey_ can also be specified (in addition to, or instead of, a global key command) for the purpose of directly activating toolbar buttons or deck key commands (such as automatically shuffling a deck at game start).

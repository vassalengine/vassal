...
If the module defines and sets _true_ one or more Global Properties of the form ``VassalExcludeSide_<side>`` each  such side will *not* appear in the side list, as if it were already taken. This may be done using Scenario Options or a standard Global Property.
|===

...
The <<Toolbar.adoc#Retire,Retire>> or <<Toolbar.adoc#SwitchSides,Switch Sides>> button, in the main controls toolbar, allows a player to relinquish their side (making it available to the next player joining the game). You can specify the text, icon, and mouse-over tooltip for the toolbar button. Leave button text and icon undefined if you define no sides.

The Switch Sides button includes a number of additional features:

*Hot Seating*. If the Module <<GlobalProperties.adoc#top,Global Property>> _VassalNextSide_ is defined and set to an available side, that side will be offered. Failing this, Vassal will determine the next side in order, excluding the non-player sides.

*Automatic Side Selection* The Switch Sides button will connect to the side specified in the <<GlobalOptions.adoc#custom,Global Options custom>> string preference property _VassalForceSide_. Vassal clears the property after each use.

*Automatic Random Side Selection* The Switch Sides button will select a random player side when a <<GlobalOptions.adoc#custom,Global Options custom>> string preference property _VassalRandomSide_ is set true. Vassal clears the property after each use.

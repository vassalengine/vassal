...
When joining a game, players will be prompted to take one of the remaining available sides. Any number of observers (players who belong to no side) are allowed.
 If more than one side remains available, players will also be offered the option to allow Vassal to select a random side, ignoring the non-player and hidden sides. Recognised non-player sides are "Solo", "Solitaire", "Moderator" and "Referee".
...
The Switch Sides button includes a number of additional features not applicable when a player first joins a game:
...
*Automatic Side Selection* The Switch Sides button will connect to the side specified in the <<GlobalProperties.adoc#top,Global Property>> _VassalForceSide_. Vassal clears the property after each use. If the side is not available, no side change is performed and a warning is displayed in the chat log.

*Automatic Random Side Selection* The Switch Sides button will select a random player side when a <<GlobalProperties.adoc#top,Global Property>> _VassalRandomSide_ is set to _true_. Vassal clears the property after each use. If no player sides are available, no side change is performed and a warning is displayed in the chat log.

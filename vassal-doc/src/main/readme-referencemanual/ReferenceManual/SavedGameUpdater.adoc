...
Running the _Refresh Predefined Setups_ tool is equivalent to running the <<GameRefresher.adoc#top, Refresh Counters Tool>> on each of the predefined setups in a module. Please see that page for detail information on the Refresher options. In addition, to assist the module developer testing a module with a large number of scenarios, a filter field is provided on the Refresh Predefine Setups panel. Use this field to test refresh on specific scenarios whilst developing a module.

...
Use these functions when you want to *Sum* or *Count* pieces that are either in the same Location as the current piece (Counter GKC's) or are in a known named Location on a specific map.
...
Use these functions when you want to *Sum* or *Count* pieces that are either in the same Zone as the current piece (Counter GKC's) or are in a known Zone on a specific map.
...
*CountZone(expression)* returns the count of the number of pieces (including the current piece) within the same Zone on the same Map as the current, piece that match the expression.

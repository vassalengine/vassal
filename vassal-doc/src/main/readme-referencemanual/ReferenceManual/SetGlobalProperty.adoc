...
*NOTE:* In certain special situations it can also allow a piece to set the value of a Dynamic Property of another piece, if the piece is attached to this one with an <<Attachment.doc#top, Attachment>> trait on the current piece (see the <<Attachment.adoc#top, Attachment>> trait for further details.)

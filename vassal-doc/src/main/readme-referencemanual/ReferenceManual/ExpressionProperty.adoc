...
Note that if your Attachment includes the 'Allow attachment to self' option, then it is possible for these functions to find and return the Property value from itself.
...
[example]
GetAttachProperty("Supplies", "SUP") +
...
[example]
GetAttachProperty("Supplies", "SUP", 4)  +
...
[example]
GetAttachProperty("Supplies", "SUP", "RU-2")  +
...
[example]
GetAttachProperty("Supplies", "SUP", "{Suppressed==false}")  +

...
NOTE: When using Grids with fully numeric Column and Row numbers with leading zeros (e.g. 0203), then Vassal will internally remove leading zeros of any Grid Reference you enter, making it impossible to Send to numeric locations that commence with a leading '0'. This can be overcome by setting the <<GlobalOptions.adoc#leadingzeros,_Store integers with leading zeros as Strings internally_ Global Option>>

...
*CountStack(prop)* returns the number of pieces in the same stack as this one, that have a non-blank value for the named property (or a count of the total pieces in the stack if a blank property is sent)

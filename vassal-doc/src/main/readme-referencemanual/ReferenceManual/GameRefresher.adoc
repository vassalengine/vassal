...
*Trigger pre and post refresh Hotkeys:*::
A pair of specially named Global Hotkeys will be issued before and after the game is refreshed. These can be used to extend the automated functionality of the game refresher, especially when running via the _Refresh Pre-Defined Setups_ tool outlined in <<SavedGameUpdater.adoc#top,this section>>. See the <<#RefreshHotkeys, Pre & Post-Refresh Hotkeys>> section below for more details.
...
When this option is checked, the Refresher will trigger special hotkeys. _VassalPreRefreshGHK_ is triggered before refreshing and _VassalPostRefreshGHK_ after refreshing. You may use this feature to perform maintenance that you wish to carry out on Predefined Setup or game files. Potential uses include converting counters or populating a new deck.

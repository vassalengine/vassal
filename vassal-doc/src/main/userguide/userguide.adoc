...
image:_images/image2.jpeg[image,width=663,height=149]
...
image:_images/image3.jpeg[image,width=484,height=303] +
...
image:_images/image5.jpeg[image,width=400,height=128] +
...
Players in a Server game must be _synchronized_ in order to see each other’s moves and interact. Players should pick a single player to synchronize with, such as the player who initiated the game. This will make sure that everyone’s game position is consistent.
...
image:_images/image6.jpeg[image,width=267,height=376] +
...
image:_images/image7.jpeg[image,width=352,height=95] +
...
clickable buttons. Sometimes, the buttons include drop-down menus with additional options. +
image:_images/image8.jpeg[image,width=701,height=77] +
...
* *Step through Log:* This button allows you to step through an email log to recap an opponent’s moves. For more information, see <<Playing by Email>>.
...
For example, to reflect damage done to a tank during combat, a tank counter can have a full-strength version and a damaged version. When the tank takes damage during combat, a player would right-click on the piece and choose *Damage* from the menu. This displays the damaged version of the tank, with reduced combat strength. In the same game, a Mine counter only has a single choice on its command menu: *Explode*. This action is selected when the mine is detonated and removes the piece from the board
...
image:_images/image9.png[image,width=198,height=160] +
...
In many games, especially war games, you can form a __stack__ by piling pieces on top of each other, then double-clicking the pile. A stack can be selected and moved like an individual piece.
...
image:_images/image10.jpeg[image,width=353,height=122] +
...
Like modules, extensions are not specific to any operating system. A module extension for Vassal 3.1 usually has the suffix .vext, although some older extensions have the suffix .mdx or even .zip.
...
. Browse to the extension file and click *Open*. The extension is now listed in the Module Manager under the name of the module. A copy of the extension file is automatically placed in a new directory called <Module Name>_ext, in the same directory where the module is located. When Vassal loads the module, any extensions in this directory are also loaded. (For example, for the module WorldWarII, upon activating the extension IwoJima.vext, the extension is placed in the directory WorldWarII_ext.)

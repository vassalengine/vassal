...
public class Resources {
	 ...
	 * 
	 * These are implemented as PropertyResourceBundles, normally to be found in
	 * the VASSAL jar file. VASSAL will search first in the VASSAL install
	 * directory for bundles, then follow the standard Java Class Path
	 */
	...
	public static String getVassalString(String id) {
		if (vassalBundle == null) {
			vassalBundle = ResourceBundle.getBundle(VASSAL_BUNDLE, Locale
					.getDefault(), new VassalPropertyClassLoader());
		}
	...
	public static String getEditorString(String id) {
		if (editorBundle == null) {
			editorBundle = ResourceBundle.getBundle(EDITOR_BUNDLE, Locale
					.getDefault(), new VassalPropertyClassLoader());
		}
	...
	/*
	 * Format a string with options. Convenience methods for most common case of
	 * one or two parameters. Will be heavily used, so try and minimise the
	 * number of Object arrays being created
	 */
	...
	/**
	 * Custom Class Loader for loading VASSAL property files. Check first for
	 * files in the VASSAL home directory
	 * 
	 * @author Brent Easton
	 * 
	 */
	public static class VassalPropertyClassLoader extends ClassLoader {
		public URL getResource(String name) {
			...
			/*
			 * No openable file in home dir, so let Java find one for us in the
			 * standard classpath.
			 */
			if (url == null) {

...
public final class Info {
	...
	/**
	 * Get size of screen accounting for the screen insets (i.e. Windows
	 * taskbar)
	 * 
	 * @return
	 */
	public static Rectangle getScreenBounds(Component c) {
		...
		bounds.setSize(bounds.width - insets.left - insets.right, bounds.height
	...
	/**
	 * 
	 * A valid verson format is "w.x.y[bz]", where 'w','x','y', and 'z' are
	 * integers.
	 * 
	 * @return a negative number if <code>v2</code> is a later version the
	 *         <code>v1</code>, a positive number if an earlier version, or
	 *         zero if the versions are the same.
	 * 
	 */
			...
			if (beta1 > 0) {
				if (beta2 > 0) {
					return compareVersions(v1.substring(0, beta1), v2
							.substring(0, beta2)) < 0 ? -1 : Integer
							.parseInt(v1.substring(beta1 + 1))
							- Integer.parseInt(v2.substring(beta2 + 1));
				} else {
					return compareVersions(v1.substring(0, beta1), v2) > 0 ? 1
							: -1;
				}
			} else if (beta2 > 0) {

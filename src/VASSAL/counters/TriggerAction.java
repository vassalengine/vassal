...
public class TriggerAction extends Decorator implements TranslatablePiece,
  ...
  /*
   * Bug 4302 requires that all commands generated by a Trigger Action, including those generated from 
   * toolbar buttons triggered by hotkeys must be guaranteed to be sent to clients and log files in the
   * same order as they are generated by the initiating client.
   * 
   * Prior to this bug fix, the counter commands generated by a Trigger Action where saved and sent in
   * order, but toolbar components triggered by hotkeys did their own thing in their own time.
   * 
   * This fix may break modules that have worked around this bug, though it is unlikely since the bug can
   * cause different actions in the two clients. It is most likely that the sequence of actions generating
   * the bug has been avoided.
   * 
   *  The fix has been coded to check the compatibilityMode variable to test whether to use the old method
   *  or the new method. At the moment, compatibilityMode is set permanently to false. If necessary, add 
   *  a new preference and set the mode based on the preference.
   */
  ...
  protected void doLoopOnce(Command c) throws RecursionLimitException {
    try {
      ...
      for (int i = 0; i < actionKeys.length && getMap() != null; i++) {
        Command lc = outer.keyEvent(actionKeys[i].getKeyStroke());
        c.append(lc);
        if (!compatibilityMode) {
          sendCommand(lc);
        }
  ...
  protected void executeKey(Command c, NamedKeyStroke key) {
    ...
    try {
      RecursionLimiter.startExecution(this);
      Command ec = outer.keyEvent(key.getKeyStroke()); 
      c.append(ec);
      if (!compatibilityMode) {
        sendCommand(ec);
      }

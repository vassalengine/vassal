...
public class ImageUtils {
  ...
  private final Map<RenderingHints.Key, Object> defaultHints = new HashMap<RenderingHints.Key, Object>();

  private ImageUtils() {
    // create configurer for memory-mappedf file preference
    final BooleanConfigurer mappedPref = new BooleanConfigurer(PREFER_MEMORY_MAPPED, "Prefer memory-mapped files for large images?", //$NON-NLS-1$
        Boolean.FALSE);
    ...
    mappedPref.addPropertyChangeListener(new PropertyChangeListener() {
      public void propertyChange(PropertyChangeEvent evt) {
        largeImageLoadMethod = Boolean.TRUE.equals(mappedPref.getValue()) ? MAPPED : RAM;
      }
    });
    GameModule.getGameModule().getPrefs().addOption(mappedPref);
    // create configurer for scaling quality
    final BooleanConfigurer scalingPref = new BooleanConfigurer(SCALER_ALGORITHM, "High-quality scaling?", //$NON-NLS-1$ 
        Boolean.TRUE);
    // Resources.getString("GlobalOptions.smooth_scaling"), Boolean.TRUE); //$NON-NLS-1$
    scalingPref.addPropertyChangeListener(new PropertyChangeListener() {
      public void propertyChange(PropertyChangeEvent evt) {
        final int newQual = Boolean.TRUE.equals(scalingPref.getValue()) ? GOOD : MEDIUM;
        if (newQual != scalingQuality) {
          scalingQuality = newQual;
          Op.clearCache();
          defaultHints.put(RenderingClues.KEY_EXT_INTERPOLATION, scalingQuality == GOOD ? RenderingClues.VALUE_INTERPOLATION_LANCZOS_MITCHELL
  ...
  public static BufferedImage transform(BufferedImage src, double scale, double angle) {
    return transform(src, scale, angle, getDefaultHints(), instance.scalingQuality);
  }
  ...
  /*
   * public static BufferedImage transform(BufferedImage src, double scale, double angle, boolean quality) { return
   * transform(src, scale, angle, getDefaultHints(), quality ? GOOD : MEDIUM); }
   */
  public static BufferedImage transform(BufferedImage src, double scale, double angle, RenderingHints hints, int quality) {
    // bail on null source
    if (src == null)
      return null;
    ...
    if (scale == 1.0 && angle % 90.0 == 0.0) {
      ...
      final BufferedImage trans = new BufferedImage(tbox.width, tbox.height, BufferedImage.TYPE_INT_ARGB);
    ...
    else if (hints.get(RenderingClues.KEY_EXT_INTERPOLATION) == RenderingClues.VALUE_INTERPOLATION_LANCZOS_MITCHELL) {
      ...
      if (angle != 0.0) {
        ...
        final BufferedImage rot = new BufferedImage(rbox.width, rbox.height, BufferedImage.TYPE_INT_ARGB);
      ...
      /*
       * // make sure that we have a TYPE_INT_ARGB before using GeneralFilter if (src.getType() !=
       * BufferedImage.TYPE_INT_ARGB) { src = toIntARGB(src); }
       */
  ...
  /*
   * private static final int type_ordering[] = new int[]{ BufferedImage.TYPE_3BYTE_BGR, BufferedImage.TYPE_4BYTE_ABGR,
   * BufferedImage.TYPE_4BYTE_ABGR_PRE, BufferedImage.TYPE_BYTE_BINARY, BufferedImage.TYPE_BYTE_GRAY,
   * BufferedImage.TYPE_BYTE_INDEXED, BufferedImage.TYPE_CUSTOM, BufferedImage.TYPE_INT_ARGB,
   * BufferedImage.TYPE_INT_ARGB_PRE, BufferedImage.TYPE_INT_BGR, BufferedImage.TYPE_INT_RGB,
   * BufferedImage.TYPE_USHORT_555_RGB, BufferedImage.TYPE_USHORT_565_RGB, BufferedImage.TYPE_USHORT_GRAY, };
   */
  ...
  public static Image getSmallImage(InputStream in) throws IOException {
    ...
    return img.getType() != BufferedImage.TYPE_INT_ARGB ? ImageUtils.toIntARGBSmall(img) : img;
  }

  public static Image getLargeImage(InputStream imageStream) throws IOException {
    BufferedImage img = getLargeBufferedImage(imageStream);
    return img.getType() != BufferedImage.TYPE_INT_ARGB ? ImageUtils.toIntARGBLarge(img) : img;
  }

  public static BufferedImage getLargeBufferedImage(InputStream imageStream) throws IOException {
    BufferedImage img = null;
    // first try preferred storage type
    int method = instance.largeImageLoadMethod;
    done: for (int tries = 0; tries < 2; ++tries, ++method) {
      final ImageInputStream stream = new FileCacheImageInputStream(imageStream, TempFileManager.getInstance().getSessionRoot());
      try {
        final Iterator<ImageReader> i = ImageIO.getImageReaders(stream);
        if (!i.hasNext())
          throw new IOException("Unrecognized image format");
        ...
        try {
          ...
          case MAPPED:
            try {
              ...
              final SampleModel sm = type.getSampleModel().createCompatibleSampleModel(w, h);
  ...
  private static BufferedImage rowByRowCopy(BufferedImage src, BufferedImage dst) {
  ...
  private static BufferedImage colorConvertCopy(BufferedImage src, BufferedImage dst) {
    final ColorConvertOp op = new ColorConvertOp(src.getColorModel().getColorSpace(), dst.getColorModel().getColorSpace(), null);
    op.filter(src, dst);
    return dst;
  }

  public static BufferedImage createEmptyLargeImage(int width, int height) {
    switch (instance.largeImageLoadMethod) {
    case MAPPED:
      try {
        return MappedBufferedImage.createIntARGBMemoryMappedImage(width, height);
      }
      catch (IOException e) {
        ErrorLog.log(e);
  ...
  public static BufferedImage toIntARGBLarge(BufferedImage src) {
    ...
    for (int tries = 0; tries < 2; ++tries, ++method) {
      ...
      case MAPPED:
        try {
          return rowByRowCopy(src, MappedBufferedImage.createIntARGBMemoryMappedImage(w, h));
        }
        catch (IOException e) {
      ...
      case RAM:
        try {
          ...
          return src instanceof MappedBufferedImage ? rowByRowCopy(src, dst) : colorConvertCopy(src, dst);
        }
        catch (OutOfMemoryError e) {
          // ignore, we throw an OutOfMemoryError at bottom
        }
  ...
  public static BufferedImage toIntARGBSmall(BufferedImage src) {
    final BufferedImage dst = new BufferedImage(src.getWidth(), src.getHeight(), BufferedImage.TYPE_INT_ARGB);
   ...
   * @throws IOException
   *           if the memory-mapped image cannot be created
   */
  public static BufferedImage toMemoryMapped(Image src) throws IOException {
    final BufferedImage dst = MappedBufferedImage.createIntARGBMemoryMappedImage(src.getWidth(null), src.getHeight(null));
